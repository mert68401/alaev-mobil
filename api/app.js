  ...
  try {
    ...
    file(req, res, async function (err) {
      ...
      if (photo) {
        ...
        const post = await database.collection("socialPosts").insertOne({
  ...
  try {
    ...
    if (post) {
      if (userId == post.SenderUser.userId) {
        ...
        fs.unlinkSync(path);
        await database.collection("socialPosts").deleteOne({ _id: ObjectId(postId) });
...
router.post("/likePost", auth, async (req, res) => {
  ...
  try {
    ...
    if (post) {
      ...
      if (!bool) {
        await database.collection("socialPosts").updateOne(
          { _id: ObjectId(postId) },
          {
            $push: {
...
router.post("/getUserAccounts", function (req, res) {
  ...
  var filter2 = {};
    if (filter.city && filter.city.length > 0) {
        filter2.city = { $in: filter.city };
    }
    if (filter.job && filter.job.length > 0) {
        filter2.job = { $in: filter.job };
    }
    if (filter.universityFaculty && filter.universityFaculty.length > 0) {
        filter2.universityFaculty = { $in: filter.universityFaculty };
    }
