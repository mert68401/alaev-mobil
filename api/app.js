                ...
                database.collection("userAccounts").updateOne(
                    ...
                    function (error, result) {
                        if (error) {
                            ...
                            sendEmail("smtp.yandex.com.tr", 587, "info@alaev.org.tr", "txjtzpiqgpvwxltn", "info@alaev.org.tr", body.email, "ALAEV", "Şifremi Unuttum", "", forgotPasswordMail(token));
                            res.send({
                                success: true,
                        ...
                        .then(function (docs) {
                            if (!docs) {
                                if (body.jobAdTitle && body.jobAdContent && body.jobType) {
                                    if (body.jobAdTitle != "" && body.jobAdContent != "" && body.jobAdMail != "") {
                                        database.collection("jobAdForms").insertOne(jobAdObj, function (err, result) {
                                            ...
                                            res.json({
                                                success: true,
                                                message: "İş ilanı oluşturuldu. Lütfen ilanınızı aktif duruma getirmek için yöneticiyle iletişime geçiniz! ",
                                            });
                                        });
                                    } else {
                                        console.log("Eksik bilgi");
                                        ...
                                        database.collection("jobAdForms").updateOne(
                                            ...
                                            function (error, result) {
                                                if (error) {
                                                    ...
                                                    res.json({
                                                        success: true,
                                                        message: "İş ilanı güncellendi. Lütfen ilanınızı aktif duruma getirmek için yöneticiyle iletişime geçiniz!",
                                                    });
                                                }
                                            }
                                        );
                                ...
                                if (body.companyAdTitle && body.companyAdCompanyNumber && body.companyAdContent) {
                                    if (body.companyAdTitle != "" && body.companyAdCompanyNumber != "" && body.companyAdContent != "") {
                                        database.collection("companyAdForms").insertOne(companyAdObj, function (err, result) {
                                            ...
                                            res.json({
                                                success: true,
                                                message: "İlan oluşturuldu. Lütfen ilanınızı aktif duruma getirmek için yöneticiyle iletişime geçiniz.",
                                            });
                                        });
                                    } else {
                                        console.log("Eksik bilgi");
                                        ...
                                        database.collection("companyAdForms").updateOne(
                                            ...
                                            function (error, result) {
                                                if (error) {
                                                    ...
                                                    res.json({
                                                        success: true,
                                                        message: "İlan güncellendi. Lütfen ilanınızı aktif duruma getirmek için yöneticiyle iletişime geçiniz.",
                                                    });
                                                }
                                            }
                                        );
...
router.post("/getJobCvData", function (req, res) {
    ...
    if (body) {
        database
            ...
            .then(function (docs) {
                if (docs) {
                    database.collection("cvForms").find({ userId: docs.userId }, function (error, result) {
                        if (error) {
                            res.status(401).send({
                                success: false,
                                message: "Cv Bulunamadı",
